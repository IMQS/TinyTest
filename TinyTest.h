...
2. #define TT_MODULE_NAME as the name of your application, and include <TinyTest.h> into all files
	that will define tests in them. The name of your application will appear in test out, such as 
	JUnit xml.

	#define TT_MODULE_NAME myprogram
	#include <TinyTest/TinyTest.h>

3. Inside one of your cpp files (typically the one containing your 'main' function), #include <TinyTest/TinyTestBuild.h>.
	This particular .cpp file is not allowed to define any tests of its own (this is a linker issue that I have not bothered to solve yet).
	Inside your main function, call TTRun. TTRun may only be called once, because it frees the test list before returning.
...
5. In other cpp files, write TT_TEST_FUNC(initfunc, teardown, size, testname, parallel) to define test functions.
	Inside your test functions, use TTASSERT().
...
enum TTChildProcessFlags
{
	TTChildProcessWait = 1 // Wait for child process to exit, before returning
};
...
// Helper function to launch a child process.
// If you set the TTChildProcessWait flag, then the return value is the exit code of the child process.
// If you don't set the TTChildProcessWait flag, then the return value is the PID of the child process,
// and TTNotifySubProcess is automatically called.
// This function panics if it fails to launch the child process.
// env may be NULL.
