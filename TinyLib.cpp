...
int TTRun_WrapperW(TT_TestList& tests, int argc, wchar_t** argv) {
	...
	if (argc >= MAXARGS - 1) {
		printf("TTRun_InternalW: Too many arguments\n");
		return 1;
	}
...
static char* CreateMergedEnvironmentBlock(const char** env) {
	if (!env)
		return nullptr;
	...
	for (size_t i = 0; env[i]; i++) {
		...
		envMap.insert({std::string(var, eq), std::string(var + eq + 1)});
	}

#ifdef _WIN32
	const char* old = GetEnvironmentStringsA();
	std::string newEnv;
	for (size_t i = 0; old[i];) {
		size_t eq = i;
		for (; old[eq] != '='; eq++) {
		}
		std::string var = std::string(old + i, eq - i);
		if (envMap.find(var) != envMap.end()) {
			// replace with new variable
			newEnv += var;
			newEnv += "=";
			newEnv += envMap.at(var);
			visited.insert(var);
		} else {
			newEnv += old + i;
		}
	...
	FreeEnvironmentStringsA((char*) old);
	old = nullptr;
	// Add extra variables passed into the function, that were not replaced
	...
	newEnv.append("\0", 1);
	char* copy = (char*) malloc(newEnv.length());
	memcpy(copy, &newEnv[0], newEnv.length());
	return copy;
#else
	std::string         newEnv;
	std::vector<size_t> newEnvOffsets;
	for (size_t i = 0; environ[i]; i++) {
		const char* oldPair = environ[i];
		size_t      eq      = 0;
		for (; oldPair[eq] != '='; eq++) {
		}
		std::string var = std::string(oldPair, eq);
		if (envMap.find(var) != envMap.end()) {
			// replace with new variable
			newEnvOffsets.push_back(newEnv.size());
			newEnv += var;
			newEnv += "=";
			newEnv += envMap.at(var);
			visited.insert(var);
	...
	// Create a packed environment block, which has the pointers first, which we can pass to execvpe,
...
bool TTLaunchChildProcess_Internal(unsigned int flags, const char* cmd, const char** args, const char** env, unsigned int& pid_or_status) {
	bool waitForExit = !!(flags & TTChildProcessWait);
#ifdef _WIN32
	std::string all = cmd;
	...
	DWORD               cpflags   = 0;
	void*               mergedEnv = CreateMergedEnvironmentBlock(env);
	...
	if (!TTIsRunningUnderMaster())
		cpflags |= CREATE_NEW_CONSOLE;
	char* buf = new char[all.size() + 1];
	memcpy(buf, all.c_str(), all.size());
	buf[all.size()] = 0;
	bool ok         = !!CreateProcessA(cmd, buf, NULL, NULL, false, cpflags, mergedEnv, NULL, &si, &pi);
	free(mergedEnv);
	if (!ok)
		return false;
	if (waitForExit) {
		...
		pid_or_status = (unsigned int) exitCode;
	} else {
		pid_or_status = pi.dwProcessId;
	}
	...
	if (pid == 0) {
		...
		if (waitForExit) {
			...
			if (!WIFEXITED(status))
				return false;
			pid_or_status = WEXITSTATUS(status);
			return true;
		} else {
...
unsigned int TTLaunchChildProcess(unsigned int flags, const char* cmd, const char** args, const char** env) {
	unsigned int pid_or_status = 0;
	bool         launched      = TTLaunchChildProcess_Internal(flags, cmd, args, env, pid_or_status);
	if (!launched) {
		...
		if (lastslash != -1 && std::string(cmd).find(DIRSEP) == -1) {
			...
			launched = TTLaunchChildProcess_Internal(flags, newCmd.c_str(), args, env, pid_or_status);
		}
	}
	...
	if (!(flags & TTChildProcessWait))
		TTNotifySubProcess(pid_or_status);

	return pid_or_status;
}
